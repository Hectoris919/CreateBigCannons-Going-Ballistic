...
public class ProjectileMassReloadListener extends SimpleJsonResourceReloadListener {
	...
	private static Optional<ProjectileMassProperties> parseProperties(ResourceLocation fileId, ResourceLocation projectileId, JsonObject json) {
		Double autocannonPowderMassKg = readOptionalPositive(fileId, projectileId, json, "autocannon_powder_mass_kg");
		Double autocannonChargeLengthMeters = readOptionalPositive(fileId, projectileId, json, "autocannon_charge_length_m");
		...
		if (GsonHelper.isNumberValue(json, "mass_kg")) {
			double massKg = GsonHelper.getAsDouble(json, "mass_kg");
			if (negative(fileId, projectileId, "mass_kg", massKg)) return Optional.empty();
		...
		if (GsonHelper.isNumberValue(json, "empty_mass_kg") && GsonHelper.isNumberValue(json, "internal_fluid_volume_m3")) {
			double emptyMassKg = GsonHelper.getAsDouble(json, "empty_mass_kg");
			...
			if (negative(fileId, projectileId, "empty_mass_kg", emptyMassKg)) return Optional.empty();
			if (negative(fileId, projectileId, "internal_fluid_volume_m3", internalVolumeM3)) return Optional.empty();
			if (negative(fileId, projectileId, "reference_fluid_density_kg_per_m3", referenceDensity)) return Optional.empty();
		...
		GoingBallistic.LOGGER.warn("Projectile mass entry {} in {} must define either mass_kg or empty_mass_kg + internal_fluid_volume_m3", projectileId, fileId);
	...
	private static Double readOptionalPositive(ResourceLocation fileId, ResourceLocation projectileId, JsonObject json, String key) {
		...
		if (negative(fileId, projectileId, key, value)) return null;
		return value;
	}

	private static boolean negative(ResourceLocation fileId, ResourceLocation projectileId, String key, double value) {
